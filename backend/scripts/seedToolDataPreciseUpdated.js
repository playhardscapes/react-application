// scripts/seedToolDataPreciseUpdated.js
...
async function seedToolData() {
  ...
  try {
    ...
    // Upsert tools and get their IDs
    const toolResults = [];
    ...
    // Clear existing maintenance and assignments for these specific tools
    await client.query(`
      DELETE FROM tool_maintenance 
      WHERE tool_id IN (${toolResults.map(t => t.id).join(',')})
    `);
